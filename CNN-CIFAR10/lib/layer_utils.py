...
class flatten(object):
    ...
    def forward(self, feat):
        ...
        #############################################################################
        output=feat.reshape(-1,feat[0].size)
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.meta = feat
...
class fc(object):
    ...
    def backward(self, dprev):
        ...
        #print(feat.reshape((feat.shape[0],self.params[self.w_name].shape[0])).T.shape)
...
class relu(object):
    ...
    def backward(self, dprev):
        ...
        # Store the output gradients in the variable dfeat provided above.          #
        #############################################################################
        dfeat = np.multiply(np.where(feat<0,0,1),dprev)
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.meta = None
...
class dropout(object):
    ...
    def forward(self, feat, is_training=True):
        ...
        #############################################################################
        if is_training:
            kept = (np.random.uniform(0.0, 1.0, feat.shape) >= self.keep_prob).astype(float)
        else:
...
class cross_entropy(object):
    ...
    def forward(self, feat, label):
        ...
        log_likelihood = -np.log(logit[range(label.shape[0]),label])
        loss= np.sum(log_likelihood)/feat.shape[0]
